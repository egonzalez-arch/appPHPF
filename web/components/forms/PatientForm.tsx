...
function adaptEmergencyContact(raw: any): ECState {
  ...
  const name =
    raw.name ?? raw.Nombre ?? raw.nombre ?? raw.fullName ?? raw.contactName ?? '';
  const relation =
    raw.relation ??
    raw.Relación ??
    raw.relacion ??
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (mode === 'create') {
      if (!form.user.firstName || !form.user.lastName || !form.user.email) {
        alert('Completa los datos de usuario (nombre, apellido, email).');
        return;
      }
    ...
    >
      ...
      {mode === 'create' && (
        <fieldset className="border rounded p-4">
          <legend className="text-sm font-medium">Datos de Usuario</legend>
      ...
      <fieldset className="border rounded p-4">
        <legend className="text-sm font-medium">Datos del Paciente</legend>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <label className="flex flex-col text-sm gap-1">
            Fecha de Nacimiento
            <input
