  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div className="flex flex-col xl:flex-row gap-3 w-full lg:w-auto lg:items-center">
          ...
          >
            ...
            {doctors?.map(d => {
              const full = [d.user?.firstName, d.user?.lastName].filter(Boolean).join(' ');
              return <option key={d.id} value={d.id}>{full || d.id}</option>;
            })}
          </select>
          ...
          >
            ...
            {patients?.map(p => {
              const full = [p.user?.firstName, p.user?.lastName].filter(Boolean).join(' ');
              return <option key={p.id} value={p.id}>{full || p.id}</option>;
            })}
          </select>
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={8} className="text-center py-8 text-gray-500">
                  {(search || doctorFilter || patientFilter || clinicFilter || statusFilter)
      ...
      <div className="md:hidden flex flex-col gap-4">
        ...
        {filtered.map(a => (
          <div key={a.id} className="border rounded shadow p-4 bg-white space-y-2">
            <div className="font-semibold text-lg">
              {findClinicName(a.clinicId)}
            </div>
            <div className="text-sm text-gray-700 space-y-1">
              <div><span className="font-medium">Doctor:</span> {findDoctorName(a.doctorId)}</div>
              <div><span className="font-medium">Paciente:</span> {findPatientName(a.patientId)}</div>
              <div><span className="font-medium">Inicio:</span> {shortDate(a.startAt)}</div>
              <div><span className="font-medium">Fin:</span> {shortDate(a.endAt)}</div>
              <div><span className="font-medium">Estado:</span> {labelForStatus(a.status)}</div>
              <div><span className="font-medium">Motivo:</span> {a.reason || '-'}</div>
            </div>
      ...
      {showConfirmCreate && pendingCreateData && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[60]">
          <div className="bg-white rounded shadow p-6 max-w-md w-full space-y-4">
            ...
            <div className="text-sm text-gray-700 space-y-1">
              <p><span className="font-medium">Clínica:</span> {findClinicName(pendingCreateData.clinicId)}</p>
              <p><span className="font-medium">Doctor:</span> {findDoctorName(pendingCreateData.doctorId)}</p>
              <p><span className="font-medium">Paciente:</span> {findPatientName(pendingCreateData.patientId)}</p>
              <p><span className="font-medium">Inicio:</span> {shortDate(pendingCreateData.startAt)}</p>
              <p><span className="font-medium">Fin:</span> {shortDate(pendingCreateData.endAt)}</p>
              <p><span className="font-medium">Motivo:</span> {pendingCreateData.reason}</p>
            </div>
            {createMut.isError && (
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[65]">
          <div className="bg-white rounded shadow p-6 max-w-md w-full space-y-4">
            ...
            <div className="text-sm">
              <p><span className="font-medium">Actual:</span> {labelForStatus(statusTarget.status)}</p>
            </div>
            <select
              value={newStatus}
            ...
            {statusMut.isError && (
              <div className="text-red-600 text-sm">
                {(statusMut.error as any)?.message || 'Error al cambiar estado'}
              </div>
            )}
